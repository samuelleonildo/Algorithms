    ...
    void bellmanFord(int s, int t)
    {
        ...
        for (size_t k = 0; k < mainSize-1; k++)
        {
            for (size_t i = 0; i < mainSize; i++)
            {
                ...
                while (j < mainSize)
                {
                    if (markArray[i].D != inf && markArray[j].D > markArray[i].D + weight(i, j))
                    {
                        markArray[j].D = markArray[i].D + weight(i, j);
                    }
                    j = next(i, j);
                }
            }
        ...
        (markArray[t].D == inf || negativeCycle) ? cout << "unreachable" : cout << markArray[t].D;
        cout << '\n';
    }
};
